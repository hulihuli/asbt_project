...
/*! layPagev1.2 - 分页插件--end */


var widthModifyFlag = 0; (function($) {
	Function.prototype.tuiExtend = function(parent, overrides) {
	...
	Function.prototype.tuiDefer = function(o, defer, args) {
		var fn = this;
	...
	window.tui = $.tuiui = {
		...
		remove: function(arg) {
			if (typeof arg == "string" || typeof arg == "number") {
				delete $.tuiui.managers[arg]
			} else {
				if (typeof arg == "object" && arg instanceof $.tuiui.core.Component) {
					delete $.tuiui.managers[arg.id]
				}
			}
		},
		get: function(arg, idAttrName) {
			idAttrName = idAttrName || "tuiuiid";
			if (typeof arg == "string" || typeof arg == "number") {
				return $.tuiui.managers[arg]
			} else {
				if (typeof arg == "object" && arg.length) {
					if (!arg[0][idAttrName] && !$(arg[0]).attr(idAttrName)) {
						return null
					}
					return $.tuiui.managers[arg[0][idAttrName] || $(arg[0]).attr(idAttrName)]
				}
			}
			return null
		},
		...
		run: function(plugin, args, ext) {
			...
			ext = $.extend({
				defaultsNamespace: "tuiDefaults",
				methodsNamespace: "tuiMethods",
			...
			ext || {});
			plugin = plugin.replace(/^tuiGet/, "");
			plugin = plugin.replace(/^tui/, "");
			if (this == null || this == window || ext.isStatic) {
				if (!$.tuiui.plugins[plugin]) {
					$.tuiui.plugins[plugin] = {
						fn: $["tui" + plugin],
						isStatic: true
					}
				}
				return new $.tuiui[ext.controlNamespace][plugin]($.extend({},
			...
			if (/Manager$/.test(plugin)) {
				return $.tuiui.get(this, ext.idAttrName)
			}
			this.each(function() {
				if (this[ext.idAttrName] || $(this).attr(ext.idAttrName)) {
					var manager = $.tuiui.get(this[ext.idAttrName] || $(this).attr(ext.idAttrName));
				...
				if (ext.hasElement) {
					new $.tuiui[ext.controlNamespace][plugin](this, p)
				} else {
					new $.tuiui[ext.controlNamespace][plugin](p)
				}
			});
			if (this.length == 0) {
				return null
			...
			if (typeof args[0] == "string") {
				var manager = $.tuiui.get(this, ext.idAttrName);
				if (manager == null) {
					return
				}
				if (args[0] == "option") {
	...
	$.tuiDefaults = {};
	$.tuiMethos = {};
	$.tuiui.defaults = $.tuiDefaults;
	$.tuiui.methods = $.tuiMethos;
	$.fn.tui = function(plugin) {
		if (plugin) {
			return $.tuiui.run.call(this, plugin, arguments)
		} else {
			return $.tuiui.get(this)
		}
	};
	$.tuiui.core.Component = function(options) {
		this.events = this.events || {};
		this.options = options || {};
		this.children = {}
	};
	$.extend($.tuiui.core.Component.prototype, {
		__getType: function() {
			return "$.tuiui.core.Component"
		},
		__idPrev: function() {
			return "tuiui"
		},
	...
	$.tuiui.core.UIComponent = function(element, options) {
		$.tuiui.core.UIComponent.base.constructor.call(this, options);
	...
	$.tuiui.core.UIComponent.tuiExtend($.tuiui.core.Component, {
		__getType: function() {
			return "$.tuiui.core.UIComponent"
		},
		_extendMethods: function() {},
		_init: function() {
			this.type = this.__getType();
			if (!this.element) {
				this.id = this.options.id || $.tuiui.getId(this.__idPrev())
			} else {
				this.id = this.options.id || this.element.id || $.tuiui.getId(this.__idPrev())
			}
			$.tuiui.add(this);
			if (!this.element) {
				return
			...
			var p = this.options;
			if ($(this.element).attr("tuiui")) {
				try {
					var attroptions = $(this.element).attr("tuiui");
		...
		_rendered: function() {
			if (this.element) {
				$(this.element).attr("tuiuiid", this.id)
			}
		},
		attr: function() {
			return []
	...
	$.tuiui.controls.Input = function(element, options) {
		$.tuiui.controls.Input.base.constructor.call(this, element, options)
	};
	$.tuiui.controls.Input.tuiExtend($.tuiui.core.UIComponent, {
		__getType: function() {
			return "$.tuiui.controls.Input"
		},
		attr: function() {
			return ["nullText"]
		},
	...
	$.tuiui.win = {
		...
		mask: function(win) {
			function setHeight() {
				if (!$.tuiui.win.windowMask) {
					return
				}
				var h = $(window).height() + $(window).scrollTop();
				$.tuiui.win.windowMask.height(h)
			}
			if (!this.windowMask) {
				this.windowMask = $("<div class='l-window-mask' style='display: block;'></div>").appendTo("body");
				$(window).bind("resize.tuiuiwin", setHeight);
		...
		unmask: function(win) {
			...
			l = jwins.length; i < l; i++) {
				var winid = jwins.eq(i).attr("tuiuiid");
				if (win && win.id == winid) {
					continue
				}
				var winmanager = $.tuiui.get(winid);
				if (!winmanager) {
					continue
				}
		...
		setFront: function(win) {
			var wins = $.tuiui.find($.tuiui.core.Win);
			for (var i in wins) {
				var w = wins[i];
				if (w == win) {
	...
	$.tuiui.core.Win = function(element, options) {
		$.tuiui.core.Win.base.constructor.call(this, element, options)
	};
	$.tuiui.core.Win.tuiExtend($.tuiui.core.UIComponent, {
		__getType: function() {
			return "$.tuiui.controls.Win"
		},
		mask: function() {
	...
	$.tuiui.toJSON = typeof JSON === "object" && JSON.stringify ? JSON.stringify: function(o) {
		var f = function(n) {
			return n < 10 ? "0" + n: n
		},
		...
		if (type === "object") {
			...
			if (o.constructor === Array) {
				for (var i = 0,
				l = o.length; i < l; i++) {
					pairs.push($.tuiui.toJSON(o[i]) || "null")
				}
				return "[" + pairs.join(",") + "]"
			}
			var name, val;
	...
	var l = $.tuiui;
	$.fn.tuiGrid = function(options) {
		return $.tuiui.run.call(this, "tuiGrid", arguments)
	};
	$.fn.tuiGetGridManager = function() {
		return $.tuiui.run.call(this, "tuiGetGridManager", arguments)
	};
	var gridHeaderHeight = 35;
	...
	$.tuiDefaults.Grid = {
		...
		alternatingRow: true,
		mouseoverRowCssClass: "tui_gird_row-over",
		enabledSort: true,
		rowAttrRender: null,
	...
	$.tuiMethos.Grid = $.tuiMethos.Grid || {};
	$.tuiDefaults.Grid.sorters = $.tuiDefaults.Grid.sorters || {};
	$.tuiDefaults.Grid.formatters = $.tuiDefaults.Grid.formatters || {};
	$.tuiDefaults.Grid.editors = $.tuiDefaults.Grid.editors || {};
	$.tuiDefaults.Grid.sorters.date = function(val1, val2) {
		return val1 < val2 ? -1 : val1 > val2 ? 1 : 0
	};
	$.tuiDefaults.Grid.sorters["int"] = function(val1, val2) {
		return parseInt(val1) < parseInt(val2) ? -1 : parseInt(val1) > parseInt(val2) ? 1 : 0
	};
	$.tuiDefaults.Grid.sorters["float"] = function(val1, val2) {
		return parseFloat(val1) < parseFloat(val2) ? -1 : parseFloat(val1) > parseFloat(val2) ? 1 : 0
	};
	$.tuiDefaults.Grid.sorters.string = function(val1, val2) {
		return String(val1).localeCompare(String(val2))
	};
	$.tuiDefaults.Grid.formatters.date = function(value, column) {
	...
	$.tuiDefaults.Grid.editors.textarea = {
		create: function(container, editParm) {
			var column = editParm.column;
			var input;
			if (column.editor.detailEdit == true) {
	...
	$.tuiDefaults.Grid.editors.select = {
		create: function(container, editParm) {
			var column = editParm.column;
	...
	$.tuiDefaults.Grid.editors.selectTree = {
		create: function(container, editParm) {
			var column = editParm.column;
	...
	$.tuiDefaults.Grid.editors.stepper = {
		create: function(container, editParm) {
			var column = editParm.column;
	...
	$.tuiui.controls.Grid = function(element, options) {
		$.tuiui.controls.Grid.base.constructor.call(this, element, options)
	};
	$.tuiui.controls.Grid.tuiExtend($.tuiui.core.UIComponent, {
		__getType: function() {
			return "$.tuiui.controls.Grid"
		},
		__idPrev: function() {
			return "grid"
		},
		_extendMethods: function() {
			return $.tuiMethos.Grid
		},
		_init: function() {
			$.tuiui.controls.Grid.base._init.call(this);
			var g = this,
			p = this.options;
			...
			if (p.detailToEdit) {
				...
				p.detail = {
					...
					onShowDetail: function(record, container, callback) {
						$(container).addClass("tui_gird_detailpanel-edit");
						g.beginEdit(record,
		...
		_render: function() {
			...
			p = this.options;
			g.grid = $(g.element);
			g.grid.addClass("tui_grid_panel");
			var gridhtmlarr = [];
			gridhtmlarr.push("        <div class='tui_panel_header'><span class='tui_panel_header_text'></span></div>");
			gridhtmlarr.push("                    <div class='tui_gird_loading'></div>");
			gridhtmlarr.push("        <div class='tui_panel_topbar'></div>");
			gridhtmlarr.push("        <div class='tui_panel_bwarp'>");
			gridhtmlarr.push("            <div class='tui_panel_body'>");
			gridhtmlarr.push("                <div class='tui_gird'>");
			gridhtmlarr.push("                    <div class='tui_gird_dragging-line'></div>");
			gridhtmlarr.push("                    <div class='tui_gird_popup'><table cellpadding='0' cellspacing='0'><tbody></tbody></table></div>");
			gridhtmlarr.push("                  <div class='tui_gird1'>");
			if (p.multihead) {
				gridhtmlarr.push("                      <div class='tui_gird_header tui_gird_header-multi'>")
			} else {
				gridhtmlarr.push("                      <div class='tui_gird_header tui_gird_header1'>")
			}
			gridhtmlarr.push("                          <div class='tui_gird_header-inner'><table class='tui_gird_header-table' cellpadding='0' cellspacing='0'><tbody></tbody></table></div>");
			gridhtmlarr.push("                      </div>");
			gridhtmlarr.push("                      <div class='tui_gird_body tui_gird_body1'>");
			gridhtmlarr.push("                      </div>");
			gridhtmlarr.push("                  </div>");
			gridhtmlarr.push("                  <div class='tui_gird2'>");
			if (p.multihead) {
				gridhtmlarr.push("                      <div class='tui_gird_header tui_gird_header-multi'>")
			} else {
				gridhtmlarr.push("                      <div class='tui_gird_header tui_gird_header2'>")
			}
			gridhtmlarr.push("                          <div class='tui_gird_header-inner'><table class='tui_gird_header-table' cellpadding='0' cellspacing='0'><tbody></tbody></table></div>");
			gridhtmlarr.push("                      </div>");
			gridhtmlarr.push("                      <div class='tui_gird_body tui_gird_body2 l-scroll'>");
			gridhtmlarr.push("                      </div>");
			gridhtmlarr.push("                  </div>");
			gridhtmlarr.push("                 </div>");
			gridhtmlarr.push("              </div>");
			...
			gridhtmlarr.push("         <div class='tui_panel_bar'>");
			gridhtmlarr.push("         <div class='tui_panel_bar_page'></div>");//加入分页
			if (p.showPageInfo) {
				gridhtmlarr.push("                      <div class='tui_bar_message'><span class='tui_bar_text'></span></div>")
			} else {
				gridhtmlarr.push("                      <div class='tui_bar_message' style='display:none;'><span class='tui_bar_text'></span></div>")
			}

			gridhtmlarr.push("         </div>");
			g.grid.html(gridhtmlarr.join(""));
			g.header = $(".tui_panel_header:first", g.grid);
			g.body = $(".tui_panel_body:first", g.grid);
			g.toolbar = $(".tui_panel_bar:first", g.grid);
			g.popup = $(".tui_gird_popup:first", g.grid);
			g.gridloading = $(".tui_gird_loading:first", g.grid);
			g.draggingline = $(".tui_gird_dragging-line", g.grid);
			g.topbar = $(".tui_panel_topbar:first", g.grid);
			g.gridview = $(".tui_gird:first", g.grid);
			g.gridview.attr("id", g.id + "grid");
			g.gridview1 = $(".tui_gird1:first", g.gridview);
			g.gridview2 = $(".tui_gird2:first", g.gridview);
			g.gridheader = $(".tui_gird_header:first", g.gridview2);
			g.gridbody = $(".tui_gird_body:first", g.gridview2);
			g.f = {};
			g.f.gridheader = $(".tui_gird_header:first", g.gridview1);
			g.f.gridbody = $(".tui_gird_body:first", g.gridview1);
			g.currentData = null;
			g.changedCells = {};
			g.editors = {};
			g.editor = {
			...
			if(p.hideHScroll){
				g.gridbody.css("overflow-x","hidden");
			}
			var pc = $.extend({},
			p);
			this._bulid();
		...
		_setFrozen: function(frozen) {
			if (frozen) {
				this.grid.addClass("tui_frozen")
			} else {
				this.grid.removeClass("tui_frozen")
			}
		},
		...
		_setHeight: function(h) {
			...
			if (h > 0) {
                    g.gridbody.height(h);
                    g.gridview.height(h + gridHeaderHeight+2);
                    if (p.hideHScroll) {
                        ...
                        if (h > 18) {
							var realHeight = g.gridbody.find(".tui_gird_body-inner").height();
							if(realHeight>h)
								g.f.gridbody.height(h - 18+1)
                        }
                    }
            }
		...
		loadData: function(loadDataParm) {
			...
			$(".tui_bar_btnload span", g.toolbar).addClass("l-disabled");
			$(".tui_gird_hd-row").removeClass("l-checked");
			if (p.dataType == "local") {
		...
		loadServerData: function(param, clause) {
			...
			var ajaxOptions = {
				...
				success: function(data) {
					...
					g._showData.tuiDefer(g, 10, [g.currentData])
				},
				complete: function() {
					...
					if (g.hasBind("loaded")) {
						...
						g.toggleLoading.tuiDefer(g, 10, [false])
					}
				},
				error: function(XMLHttpRequest, textStatus, errorThrown) {
					...
					g.currentData[p.record] = g.data[p.record] = 0;
					g.toggleLoading.tuiDefer(g, 10, [false]);
					$(".tui_bar_btnload span", g.toolbar).removeClass("l-disabled");
		...
		deleteSelectedRow: function() {
			...
			for (var i in this.selected) {
				...
				if (o.__id in this.records) {
					this._deleteData.tuiDefer(this, 10, [o])
				}
			}
			this.reRender.tuiDefer(this, 20)
		},
		...
		setCellEditing: function(rowdata, column, editing) {
			...
			$(cell)[methodName]("tui_gird_row-cell-editing");
			if (rowdata.__id != 0) {
				...
				$(cellprev)[methodName]("tui_gird_row-cell-editing-topcell")
			}
			if (column.__previd != -1 && column.__previd != null) {
				var cellprev = $(g.getCellObj(rowdata, column)).prev();
				$(cellprev)[methodName]("tui_gird_row-cell-editing-leftcell")
			}
		},
		reRender: function(e) {
			var g = this,
		...
		changePage: function(ctype) {
			...
			if (p.newPage == 1) {
				$(".tui_bar_btnfirst span", g.toolbar).addClass("l-disabled");
				$(".tui_bar_btnprev span", g.toolbar).addClass("l-disabled")
			} else {
				$(".tui_bar_btnfirst span", g.toolbar).removeClass("l-disabled");
				$(".tui_bar_btnprev span", g.toolbar).removeClass("l-disabled")
			}
			if (p.newPage == p.pageCount) {
				$(".tui_bar_btnlast span", g.toolbar).addClass("l-disabled");
				$(".tui_bar_btnnext span", g.toolbar).addClass("l-disabled")
			} else {
				$(".tui_bar_btnlast span", g.toolbar).removeClass("l-disabled");
				$(".tui_bar_btnnext span", g.toolbar).removeClass("l-disabled")
			}
		...
		toggleCol: function(columnparm, visible, toggleByPopup) {
			...
			if (!toggleByPopup) {
				$(":checkbox[columnindex=" + columnindex + "]", g.popup).each(function() {
					...
					if ($.fn.tuiCheckBox) {
						var checkboxmanager = $(this).tuiGetCheckBoxManager();
						if (checkboxmanager) {
		...
		setColumnWidth: function(columnparm, newwidth) {
			...
			$(cells).css("width", newwidth).find("> div.tui_gird_row-cell-inner:first").css("width", newwidth - 8);
			g._updateFrozenWidth();
		...
		changeHeaderText: function(columnparm, headerText) {
			...
			var columnindex = column.__leafindex;
			var headercell = document.getElementById(column.__domid);
			$(".tui_gird_hd-cell-text", headercell).html(headerText);
		...
		resetWidth: function() {
			...
			if (p.percentWidthMode == true) {
				if (p.oldTotalWidth != g.grid.width()) {
					...
					g.gridtablewidth = g.grid.width() - rownumbersColFix - checkboxColFix-2;
					if (g.columns) {
		...
		setScroller: function(scrollY, scrollX, animate) {
			...
			if (scrollY && typeof(scrollY) == "string") {
				if (scrollY == "bottom") {
					scrollYNum = g.gridbody.find(".tui_gird_body-inner").height()
				} else {
					if (scrollY == "top") {
						scrollYNum = 0
					}
			...
			if (scrollX && typeof(scrollX) == "string") {
				if (scrollX == "right") {
					scrollXNum = g.gridbody.find(".tui_gird_body-inner").width()
				} else {
					if (scrollX == "left") {
						scrollXNum = 0
					}
		...
		collapseDetail: function(rowParm) {
			...
			l = g.columns.length; i < l; i++) {
				if (g.columns[i].isdetail) {
					...
					$(row).next("tr.tui_gird_detailpanel").hide();
					$(".tui_gird_row-cell-detailbtn:first", cell).removeClass("l-open");
					g.trigger("SysGridHeightChanged");
					return
				}
			}
		...
		extendDetail: function(rowParm) {
			...
			l = g.columns.length; i < l; i++) {
				if (g.columns[i].isdetail) {
					var row = g.getRowObj(rowdata);
					var cell = g.getCellObj(rowdata, g.columns[i]);
					$(row).next("tr.tui_gird_detailpanel").show();
					$(".tui_gird_row-cell-detailbtn:first", cell).addClass("l-open");
					g.trigger("SysGridHeightChanged");
					return
				}
			}
		...
		upgrade: function(rowParm) {
			...
			$("> td > div > .tui_gird_tree-space:last", rowobjs).addClass("tui_gird_tree-link tui_gird_tree-link-open")
		},
		demotion: function(rowParm) {
			...
			$("> td > div > .tui_gird_tree-space:last", rowobjs).removeClass("tui_gird_tree-link tui_gird_tree-link-open tui_gird_tree-link-close");
		...
		collapse: function(rowParm) {
			...
			var linkbtn = $(".tui_gird_tree-link", targetRowObj);
			if (linkbtn.hasClass("tui_gird_tree-link-close")) {
				return
			}
			g.toggle(rowParm)
		},
		expand: function(rowParm) {
			...
			var targetRowObj = g.getRowObj(rowParm);
			var linkbtn = $(".tui_gird_tree-link", targetRowObj);
			if (linkbtn.hasClass("tui_gird_tree-link-open")) {
				return
			}
			g.toggle(rowParm)
		},
		toggle: function(rowParm) {
			...
			var linkbtn = $(".tui_gird_tree-link:first", targetRowObj);
			var opening = true;
			g.collapsedRows = g.collapsedRows || [];
			if (linkbtn.hasClass("tui_gird_tree-link-close")) {
				linkbtn.removeClass("tui_gird_tree-link-close").addClass("tui_gird_tree-link-open");
				...
				opening = false;
				linkbtn.addClass("tui_gird_tree-link-close").removeClass("tui_gird_tree-link-open");
				indexInCollapsedRows = $.inArray(rowdata, g.collapsedRows);
				if (indexInCollapsedRows == -1) {
			...
			l = children.length; i < l; i++) {
				...
				if (opening) {
					$(".tui_gird_tree-link", currentRow).removeClass("tui_gird_tree-link-close").addClass("tui_gird_tree-link-open");
					currentRow.show()
				} else {
					$(".tui_gird_tree-link", currentRow).removeClass("tui_gird_tree-link-open").addClass("tui_gird_tree-link-close");
					currentRow.hide()
				}
			}
		},
		...
		_initBuildHeader: function() {
			...
			p = this.options;
			if (p.title) {
				$(".tui_panel_header_text", g.header).html(p.title);
				if (p.headerImg) {
					g.header.append("<img src='" + p.headerImg + "' />").addClass("tui_panel_header_hasicon")
				}
			} else {
				g.header.hide()
			}
			if (p.toolbar) {
				if ($.fn.tuiToolBar) {
					g.toolbarManager = g.topbar.tuiToolBar(p.toolbar)
				}
			} else {
				g.topbar.remove()
			}
		...
		_createHeaderCell: function(column) {
			var g = this,
			p = this.options;
			var jcell = $("<td class='tui_gird_hd-cell'><div class='tui_gird_hd-cell-inner'><span class='tui_gird_hd-cell-text'></span></div></td>");
			jcell.attr("id", column.__domid);
			if (!column.__leaf) {
				jcell.addClass("tui_gird_hd-cell-mul")
			}
			if (column.columnindex == g.columns.length - 1) {
				jcell.addClass("tui_gird_hd-cell-last")
			}
			if (column.isrownumber) {
				jcell.addClass("tui_gird_hd-cell-rownumbers");
				jcell.html("<div class='tui_gird_hd-cell-inner'></div>")
			}
			if (column.ischeckbox) {
				jcell.addClass("tui_gird_hd-cell-checkbox");
				jcell.html("<div class='tui_gird_hd-cell-inner'><span class='tui_gird_hd-cell-text tui_gird_hd-cell-btn-checkbox'></span></div>")
			}
			if (column.isdetail) {
				jcell.addClass("tui_gird_hd-cell-detail");
				jcell.html("<div class='tui_gird_hd-cell-inner'><div class='tui_gird_hd-cell-text tui_gird_hd-cell-btn-detail'></div></div>")
			}
			if (column.heightAlign) {
				$(".tui_gird_hd-cell-inner:first", jcell).css("textAlign", column.heightAlign)
			}
			if (column.__colSpan) {
			...
			$(".tui_gird_hd-cell-text:first", jcell).html(headerText);
			if (p.enabledSort) {
                  $(".tui_gird_hd-cell-text:first", jcell).css("cursor", "pointer")
            }
			if (!column.issystem && column.__leaf && column.resizable !== false && $.fn.tuiResizable) {
				g.colResizable[column.__id] = jcell.tuiResizable({
					handles: "e",
		...
		_initBuildGridHeader: function() {
			...
			for (var level = 1; level <= g._columnMaxLevel; level++) {
				...
				var tr = $("<tr class='tui_gird_hd-row'></tr>");
				var trf = $("<tr class='tui_gird_hd-row'></tr>");
				if (!islast) {
					tr.add(trf).addClass("tui_gird_hd-mul")
				}
		...
		_initBuildPopup: function() {
			...
			if ($.fn.tuiCheckBox) {
				$("input:checkbox", g.popup).tuiCheckBox({
		...
		_fillGridBody: function(data, frozen) {
			var g = this,
			p = this.options;
			var gridhtmlarr = ['<div class="tui_gird_body-inner"><table class="tui_gird_body-table" cellpadding=0 cellspacing=0><tbody>'];
			if (g.enabledGroup()) {
				var groups = [];
				var groupsdata = [];
				g.groups = groupsdata;
				...
				$(groupsdata).each(function(i, item) {
					if (groupsdata.length == 1) {
						gridhtmlarr.push('<tr class="tui_gird_grouprow tui_gird_grouprow-last tui_gird_grouprow-first"')
					}
					if (i == groupsdata.length - 1) {
						gridhtmlarr.push('<tr class="tui_gird_grouprow tui_gird_grouprow-last"')
					} else {
						if (i == 0) {
							gridhtmlarr.push('<tr class="tui_gird_grouprow tui_gird_grouprow-first"')
						} else {
							gridhtmlarr.push('<tr class="tui_gird_grouprow"')
						}
					}
					gridhtmlarr.push(' groupindex"=' + i + '" >');
					gridhtmlarr.push('<td colSpan="' + g.columns.length + '" class="tui_gird_grouprow-cell">');
					gridhtmlarr.push('<span class="tui_gird_group-togglebtn">&nbsp;&nbsp;&nbsp;&nbsp;</span>');
					if (p.groupRender) {
					...
					if (g.isTotalSummary()) {
						gridhtmlarr.push(g._getTotalSummaryHtml(item, "tui_gird_totalsummary-group", frozen))
					}
				})
			} else {
		...
		_showData: function() {
			...
			$(".tui_bar_btnloading:first", g.toolbar).removeClass("tui_bar_btnloading");
			...
			$(".tui_bar_btnload:first span", g.toolbar).removeClass("l-disabled");
			g._updateGridData();
			...
			g.trigger("SysGridHeightChanged");
			if (p.totalRender) {
				$(".tui_panel_bar-total", g.element).remove();
				$(".tui_panel_bar", g.element).before('<div class="tui_panel_bar-total">' + p.totalRender(g.data, g.filteredData) + "</div>")
			}
		...
		_getHtmlFromData: function(data, frozen) {
			...
			for (var rowparm in data) {
				...
				gridhtmlarr.push(' class="tui_gird_row');
				...
				if (item.__index % 2 == 1 && p.alternatingRow) {
					gridhtmlarr.push(" tui_gird_row-alt")
				}
				...
				$(g.columns).each(function(columnindex, column) {
					...
					if (this.isrownumber) {
						gridhtmlarr.push(' class="tui_gird_row-cell tui_gird_row-cell-rownumbers" style="width:' + this.width + 'px"><div class="tui_gird_row-cell-inner"');
					...
					if (this.ischeckbox) {
						gridhtmlarr.push(' class="tui_gird_row-cell tui_gird_row-cell-checkbox" style="width:' + this.width + 'px"><div class="tui_gird_row-cell-inner"');
						if (p.fixedCellHeight) {
							gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ')
						}
						gridhtmlarr.push('><span class="tui_gird_row-cell-btn-checkbox"></span></div></td>');
						return
					} else {
						if (this.isdetail) {
							gridhtmlarr.push(' class="tui_gird_row-cell tui_gird_row-cell-detail" style="width:' + this.width + 'px"><div class="tui_gird_row-cell-inner"');
							if (p.fixedCellHeight) {
								gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ')
							}
							gridhtmlarr.push('><span class="tui_gird_row-cell-detailbtn"></span></div></td>');
							return
						}
					}
					rightContent = 1;
					var colwidth = this._width;
					gridhtmlarr.push(' class="tui_gird_row-cell ');
					if (g.changedCells[rowid + "_" + this["__id"]]) {
						gridhtmlarr.push("tui_gird_row-cell-edited ")
					}
					if (this.islast) {
						gridhtmlarr.push("tui_gird_row-cell-last ")
					}
					gridhtmlarr.push('"');
		...
		_getCellHtml: function(rowdata, column) {
			...
			p = this.options;
			if (column.isrownumber) {
				return '<div class="tui_gird_row-cell-inner">' + (parseInt(rowdata.__index) + 1) + "</div>"
			}
			var htmlarr = [];
			htmlarr.push('<div class="tui_gird_row-cell-inner"');
		...
		_getTreeCellHtml: function(oldContent, rowdata) {
			...
			for (var i = 1; i < level; i++) {
				content += "<div class='tui_gird_tree-space'></div>"
			}
			if (isExtend && isParent) {
				content += "<div class='tui_gird_tree-space tui_gird_tree-link tui_gird_tree-link-open'></div>"
			} else {
				if (isParent) {
					content += "<div class='tui_gird_tree-space tui_gird_tree-link tui_gird_tree-link-close'></div>"
				} else {
					content += "<div class='tui_gird_tree-space'></div>"
				}
			}
			content += "<span class='tui_gird_tree-content'>" + oldContent + "</span>";
			return content
		},
		_applyEditor: function(obj) {
			...
			if (column.editor.type && p.editors[column.editor.type]) {
				...
				var container = $("<div class='tui_gird_editor'></div>").appendTo("body");
				if ($.browser.mozilla) {
					container.css({
						left: offset.left,
						top: offset.top
				...
				function() {
					...
					if (newValue != currentdata) {
						$(rowcell).addClass("tui_gird_row-cell-edited");
		...
		_getTotalSummaryHtml: function(data, classCssName, frozen) {
			...
			var totalsummaryArr = [];
			if (classCssName) {
				totalsummaryArr.push('<tr class="tui_gird_totalsummary ' + classCssName + '">')
			} else {
				totalsummaryArr.push('<tr class="tui_gird_totalsummary">')
			}
			$(g.columns).each(function(columnindex, column) {
				...
				if (this.isrownumber) {
					totalsummaryArr.push('<td class="tui_gird_totalsummary-cell tui_gird_totalsummary-cell-rownumbers" style="width:' + this.width + 'px"><div>&nbsp;</div></td>');
					return
				}
				if (this.ischeckbox) {
					totalsummaryArr.push('<td class="tui_gird_totalsummary-cell tui_gird_totalsummary-cell-checkbox" style="width:' + this.width + 'px"><div>&nbsp;</div></td>');
					return
				} else {
					if (this.isdetail) {
						totalsummaryArr.push('<td class="tui_gird_totalsummary-cell tui_gird_totalsummary-cell-detail" style="width:' + this.width + 'px"><div>&nbsp;</div></td>');
						return
					}
				}
				totalsummaryArr.push('<td class="tui_gird_totalsummary-cell');
				if (this.islast) {
					totalsummaryArr.push(" tui_gird_totalsummary-cell-last")
				}
				...
				totalsummaryArr.push('><div class="tui_gird_totalsummary-cell-inner"');
				if (column.align) {
		...
		_buildPager: function() {
			...
			//初始化分页
			 laypage({
				cont: $('.tui_panel_bar_page', g.toolbar),
				curr:p.page,
				pages: p.pageCount, //总页数
				skip: true, //是否开启跳页
				groups: 3, //连续显示分页数
				jump: function(e, first){ //触发分页后的回调
					if(!first){ //一定要加此判断，否则初始时会无限刷新
						p.newPage = e.curr;
						if (p.dataAction == "server") {
								setTimeout(function (){g.loadData(p.where)}, 100);
						} else {
							g.currentData = g._getCurrentPageData(g.filteredData);
							setTimeout(function (){g._showData()}, 100);
						}						
					}
				}
			});
			...
			//添加刷新按钮
			if (p.dataAction == "server") {
				var loadBtn = $(".tui_panel_bar_page .laypage_total", g.toolbar).after('<div class="tui_bar_button tui_bar_btnload"><span class=""></span></div>');
				//loadBtn.bind("click",function(){g.loadData(p.where)})
			}
			...
			$(".tui_bar_text", g.toolbar).html(stat);
		},
		...
		_getSrcElementByEvent: function(e) {
			...
			if (fn("tui_gird_editor")) {
				return {
					editing: true,
					editor: fn("tui_gird_editor")
				}
			}
			...
			var indetail = false;
			if (jobjs.hasClass("tui_gird_detailpanel") && g.detailrows) {
				for (var i = 0,
			...
			var r = {
				grid: fn("tui_grid_panel"),
				indetail: indetail,
				frozen: fn(g.gridview1[0]) ? true: false,
				header: fn("tui_panel_header"),
				gridheader: fn("tui_gird_header"),
				gridbody: fn("tui_gird_body"),
				total: fn("tui_panel_bar-total"),
				popup: fn("tui_gird_popup"),
				toolbar: fn("tui_panel_bar")
			};
			if (r.gridheader) {
				r.hrow = fn("tui_gird_hd-row");
				r.hcell = fn("tui_gird_hd-cell");
				r.hcelltext = fn("tui_gird_hd-cell-text");
				r.checkboxall = fn("tui_gird_hd-cell-checkbox");
				if (r.hcell) {
			...
			if (r.gridbody) {
				r.row = fn("tui_gird_row");
				r.cell = fn("tui_gird_row-cell");
				r.checkbox = fn("tui_gird_row-cell-btn-checkbox");
				r.groupbtn = fn("tui_gird_group-togglebtn");
				r.grouprow = fn("tui_gird_grouprow");
				r.detailbtn = fn("tui_gird_row-cell-detailbtn");
				r.detailrow = fn("tui_gird_detailpanel");
				r.totalrow = fn("tui_gird_totalsummary");
				r.totalcell = fn("tui_gird_totalsummary-cell");
				r.rownumberscell = $(r.cell).hasClass("tui_gird_row-cell-rownumbers") ? r.cell: null;
				r.detailcell = $(r.cell).hasClass("tui_gird_row-cell-detail") ? r.cell: null;
				r.checkboxcell = $(r.cell).hasClass("tui_gird_row-cell-checkbox") ? r.cell: null;
				r.treelink = fn("tui_gird_tree-link");
				r.editor = fn("tui_gird_editor");
				if (r.row) {
					r.data = this._getRowByDomId(r.row.id)
				}
				if (r.cell) {
					r.editing = $(r.cell).hasClass("tui_gird_row-cell-editing")
				}
				if (r.editor) {
					r.editing = true
				}
			...
			if (r.toolbar) {
				r.first = fn("tui_bar_btnfirst");
				r.last = fn("tui_bar_btnlast");
				r.next = fn("tui_bar_btnnext");
				r.prev = fn("tui_bar_btnprev");
				r.load = fn("tui_bar_btnload");
				r.button = fn("tui_bar_button")
			}
			return r
		},
		_setEvent: function() {
			...
			function(e) {
				//return g._onContextmenu.call(g, e)  去除了右健菜单
			});
			...
			//	g.changePage("input")
			//});
		//	$("div.tui_bar_button", g.toolbar).hover(function() {
			//	$(this).addClass("tui_bar_button-over")
		//	},
		//	function() {
		////		$(this).removeClass("tui_bar_button-over")
		//	});
			if ($.fn.tuiDrag && p.colDraggable) {
				...
				g.gridheader.add(g.f.gridheader).tuiDrag({
					revert: true,
					animate: false,
					proxyX: 0,
					proxyY: 0,
					proxy: function(draggable, e) {
						...
						if (src.hcell && src.column) {
							var content = $(".tui_gird_hd-cell-text:first", src.hcell).html();
					...
					onStopDrag: function(current, e) {
						...
						g.coldragging = false;
						if (g.colDropIn != -1) {
							g.changeCol.tuiDefer(g, 0, [column, g.colDropIn, g.colDropDir == "right"]);
							g.colDropIn = -1
						}
						g.colDroptip.hide();
						this.set("cursor", "default")
		...
		_onClick: function(e) {
			...
			var src = g._getSrcElementByEvent(e);
			if (src.out) {
				if (g.editor.editing && !$.tuiui.win.masking) {
					g.endEdit()
				}
				if (p.allowHideColumn) {
					g.popup.hide()
			...
			if (src.checkboxall) {
				...
				if (src.hcelltext) {
					...
					var sort = $(".tui_gird_hd-cell-sort:first", hcell);
					var columnName = src.column.name;
					if (!columnName) {
						return
					}
					if (sort.length > 0) {
						if (sort.hasClass("tui_gird_hd-cell-sort-asc")) {
							sort.removeClass("tui_gird_hd-cell-sort-asc").addClass("tui_gird_hd-cell-sort-desc");
							hcell.removeClass("tui_gird_hd-cell-asc").addClass("tui_gird_hd-cell-desc");
							g.changeSort(columnName, "desc")
						} else {
							if (sort.hasClass("tui_gird_hd-cell-sort-desc")) {
								sort.removeClass("tui_gird_hd-cell-sort-desc").addClass("tui_gird_hd-cell-sort-asc");
								hcell.removeClass("tui_gird_hd-cell-desc").addClass("tui_gird_hd-cell-asc");
								g.changeSort(columnName, "asc")
							}
						}
					} else {
						hcell.removeClass("tui_gird_hd-cell-desc").addClass("tui_gird_hd-cell-asc");
						$(src.hcelltext).after("<span class='tui_gird_hd-cell-sort tui_gird_hd-cell-sort-asc'>&nbsp;&nbsp;</span>");
						g.changeSort(columnName, "asc")
					}
					$(".tui_gird_hd-cell-sort", g.gridheader).add($(".tui_gird_hd-cell-sort", g.f.gridheader)).not($(".tui_gird_hd-cell-sort:first", hcell)).remove()
				} else {
					if (src.detailbtn && p.detail) {
						...
						if ($(src.detailbtn).hasClass("l-open")) {
							if (p.detail.onCollapse) {
								p.detail.onCollapse(item, $(".tui_gird_detailpanel-inner:first", nextrow)[0])
							}
							row.next("tr.tui_gird_detailpanel").hide();
							$(src.detailbtn).removeClass("l-open")
						} else {
							var nextrow = row.next("tr.tui_gird_detailpanel");
							if (nextrow.length > 0) {
								nextrow.show();
								if (p.detail.onExtend) {
									p.detail.onExtend(item, $(".tui_gird_detailpanel-inner:first", nextrow)[0])
								}
							...
							var detailRow = $("<tr class='tui_gird_detailpanel'><td><div class='tui_gird_detailpanel-inner' style='display:none'></div></td></tr>");
							var detailFrozenRow = $("<tr class='tui_gird_detailpanel'><td><div class='tui_gird_detailpanel-inner' style='display:none'></div></td></tr>");
						...
						if (src.groupbtn) {
							...
							var opening = true;
							if ($(src.groupbtn).hasClass("tui_gird_group-togglebtn-close")) {
								$(src.groupbtn).removeClass("tui_gird_group-togglebtn-close");
								if (grouprow.hasClass("tui_gird_grouprow-last")) {
									$("td:first", grouprow).width("auto")
								}
							} else {
								opening = false;
								$(src.groupbtn).addClass("tui_gird_group-togglebtn-close");
								if (grouprow.hasClass("tui_gird_grouprow-last")) {
									$("td:first", grouprow).width(g.gridtablewidth)
								}
							}
							var currentRow = grouprow.next(".tui_gird_row,.tui_gird_totalsummary-group,.tui_gird_detailpanel");
							while (true) {
								...
								if (opening) {
									currentRow.show();
									if (currentRow.hasClass("tui_gird_detailpanel") && !currentRow.prev().find("td.tui_gird_row-cell-detail:first span.tui_gird_row-cell-detailbtn:first").hasClass("l-open")) {
										currentRow.hide()
									}
								} else {
									currentRow.hide()
								}
								currentRow = currentRow.next(".tui_gird_row,.tui_gird_totalsummary-group,.tui_gird_detailpanel")
							}
							g.trigger("SysGridHeightChanged")
						} else {
							if (src.treelink) {
	...
	$.tuiui.controls.Grid.prototype.enabledTotal = $.tuiui.controls.Grid.prototype.isTotalSummary;
	$.tuiui.controls.Grid.prototype.add = $.tuiui.controls.Grid.prototype.addRow;
	$.tuiui.controls.Grid.prototype.update = $.tuiui.controls.Grid.prototype.updateRow;
	$.tuiui.controls.Grid.prototype.append = $.tuiui.controls.Grid.prototype.appendRow;
	$.tuiui.controls.Grid.prototype.getSelected = $.tuiui.controls.Grid.prototype.getSelectedRow;
	$.tuiui.controls.Grid.prototype.getSelecteds = $.tuiui.controls.Grid.prototype.getSelectedRows;
	$.tuiui.controls.Grid.prototype.getCheckedRows = $.tuiui.controls.Grid.prototype.getSelectedRows;
	$.tuiui.controls.Grid.prototype.getCheckedRowObjs = $.tuiui.controls.Grid.prototype.getSelectedRowObjs;
	$.tuiui.controls.Grid.prototype.setOptions = $.tuiui.controls.Grid.prototype.set
})(jQuery); (function(a) {
	a.fn.tuiResizable = function(b) {
		return a.tuiui.run.call(this, "tuiResizable", arguments, {
			idAttrName: "tuiuiresizableid",
			hasElement: false,
			propertyToElemnt: "target"
		})
	};
	a.fn.tuiGetResizableManager = function() {
		return a.tuiui.run.call(this, "tuiGetResizableManager", arguments, {
			idAttrName: "tuiuiresizableid",
			hasElement: false,
			propertyToElemnt: "target"
		})
	};
	a.tuiDefaults.Resizable = {
	...
	a.tuiui.controls.Resizable = function(b) {
		a.tuiui.controls.Resizable.base.constructor.call(this, null, b)
	};
	a.tuiui.controls.Resizable.tuiExtend(a.tuiui.core.UIComponent, {
		__getType: function() {
			return "Resizable"
		},
		__idPrev: function() {
		...
		_render: function() {
			...
			b.target.mousemove(function(f) {
				...
				if (c.target.tuiuidragid) {
					var d = a.tuiui.get(c.target.tuiuidragid);
					if (d && b.dir) {
						d.set("disabled", true)
					} else {
						if (d) {
		...
		_rendered: function() {
			this.options.target.tuiuiresizableid = this.id
		},
		_getDir: function(k) {
			var j = this,
			b = this.options;
	...
	a.fn.tuiToolBar = function(b) {
		return a.tuiui.run.call(this, "tuiToolBar", arguments)
	};
	a.fn.tuiGetToolBarManager = function() {
		return a.tuiui.run.call(this, "tuiGetToolBarManager", arguments)
	};
	a.tuiDefaults.ToolBar = {};
	a.tuiMethos.ToolBar = {};
	a.tuiui.controls.ToolBar = function(c, b) {
		a.tuiui.controls.ToolBar.base.constructor.call(this, c, b)
	};
	a.tuiui.controls.ToolBar.tuiExtend(a.tuiui.core.UIComponent, {
		__getType: function() {
			return "ToolBar"
		},
		__idPrev: function() {
		...
		addItem: function(e) {
			...
			if (e.line) {
				if(f.position=="right"){
					d.toolBar.append('<div class="tui_bar_separator-right"></div>');
				}
				else{
					d.toolBar.append('<div class="tui_bar_separator"></div>');
				}
				return
			}
			var b = a('<div class="l-toolbar-item tui_panel_btn"></div>');
			d.toolBar.append(b);
			...
			var itemStr = '<a class="btn size_MINI radius">';
			if(e.iconClass){
				itemStr = itemStr+'<i class="'+e.iconClass+'"></i>&nbsp;';
			}
			itemStr+='</a>';
			c = a(itemStr);
			b.append(c);
			...
			b.hover(function() {
				a(this).addClass("tui_panel_btn-over")
			},
			function() {
				a(this).removeClass("tui_panel_btn-over")
			})
		}
	})
})(jQuery);