...
Page.prototype.view = function(){
    ...
    //------------------------------------------------------------------------------
    conf.limits = "object" == typeof conf.limits ? conf.limits: [10, 20, 30, 40, 50];
    conf.limit = conf.limit || 10 ;
    conf.pages = Math.ceil(conf.all / conf.limit) || conf.pages;
    conf.all =  conf.all;
    ...
    return '<div name="laypage'+ laypage.v +'" class="laypage_main laypageskin_'+ (conf.skin ? (function(skin){ 
    		return /^#/.test(skin) ? 'molv' : skin;}(conf.skin)) : 'default')
   			+'" id="laypage_'+ that.config.item +'">'+ view.join('') + function(){
       		return conf.skip ? '<span class="laypage_total"><label>&#x5230;&#x7B2C;</label><input type="number" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" class="laypage_skip"><label>&#x9875;</label>'
        						+ '<button type="button" class="laypage_btn">&#x786e;&#x5b9a;</button></span>' 
      							: ''
			//---------------------------------------------------------------
    		}() + function(){ 
    			return conf.all ? function(){
    				var e= ['<span class="layui-laypage-limits"><select lay-ignore>'];
    				 return $.each(conf.limits,
                        function(t, n) {
//                      	console.log(n == conf.limit )
                            e.push('<option value="' + n + '"' + (n == conf.limit ? "selected": "") + ">" + n + " 条/页</option>")
                        }),
                    e.join("") + "</select></span>" 
    			}()+'<span class="layui-laypage-count">共' + conf.all + '条</span>' 
    			: ''
    		}()
    		+'</div>';
};
...
//跳页
Page.prototype.jump = function(elem,a){
    if(!elem) return;
    ...
    var	c = function() {
            var e = 0 | input.value.replace(/\s|\D/g, "");
            e && e <= conf.pages && (conf.curr = e, that.render())
        };
    if(a) return c();
    ...
    if(select){
    	Page.on(select,'change',function(){
    		var e = this.value;
            conf.pages * e > conf.all && (conf.pages = Math.ceil(conf.all / e)),
                conf.limit = e,
                conf.curr = 1,
//              console.log(conf.pages+":"+conf.limit),
                that.render()
    	})
    }
    if(btn){
...
Page.prototype.render = function(load){
    ...
    var view = that.view();
    $(conf.cont).addClass("laypage_main_box");
    if(conf.pages==0){
		$(conf.cont).append("<div class='laypage_main_nodata'>暂无数据</div>")
		return;
	}
    if(type === 2){
...
//  if(conf.all){
//		$(conf.cont).append("<div class='laypage_main_desc'>共查询到"+conf.all+"条记录，每页"+conf.pernum+"条</div>")
//	}
};
